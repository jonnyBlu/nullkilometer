...
//$(document).ready(function () {
	...
	$(".openingDayContainer").find(".point_of_sale_opening_times_from").each(function(){
			...
			if( value != ""){
				...
				var checkboxToCheck = $(this).parent().find(".point_of_sale_opening_times_dayId input[type='checkbox']#"+id);
...
//				checkboxToCheck.prop("checked",true).trigger("change"); // does not work!!!!
	...
	}, "Please specify at least one opening day");

	jQuery.validator.addMethod("openingTimes", function(value, element) {
		return validateOpeningTimes($(element).attr("id"));	
	}, "Please specify opening time(s) or deselect the day");


	var validateOpeningDays = function(){
	...
	var validateOpeningTimes = function(parentId){
